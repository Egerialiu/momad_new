...
class NuscMapExtractor(object):
    ...
    def __init__(self, data_root: str, roi_size: Union[List, Tuple]) -> None:
        ...
        for loc in self.MAPS:
            try:
                ...
                print(f"Warning: Failed to load map {loc}: {e}. Skipping map extraction.")
    ...
    def get_map_geom(self,
        ...
        # If map explorer not available for this location, return empty annotations
        if location not in self.map_explorer or self.map_explorer[location] is None:
            return dict(
                divider=[],
                ped_crossing=[],
                boundary=[],
        ...
        if location not in self.map_explorer or self.map_explorer[location] is None:
            return dict(divider=[], ped_crossing=[], boundary=[], drivable_area=[])
